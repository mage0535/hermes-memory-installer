...
| /root/.hermes/state.db | 3.9 GB | Core data | **VACUUM** (est. save 0.5-1.5G) |
| /root/backups/hermes-memory-cutover-20260625_1810/state.db.sqlite3 | 3.4 GB | **Old backup** | ✅ **DELETE** |
| /root/.cache/ms-playwright/ | 3.1 GB | Playwright browsers | Keep (needed) |
| /var/lib/docker/ | 2.4 GB | Docker data | **Prune** (limited, images active) |
| /root/.cache/pip/ | 1.8 GB | Pip cache | ✅ **PURGE** |
| /tmp/hermes-memory-backup-20260628-020655.tar.gz | 1.5 GB | **Old temp backup** | ✅ **DELETE** |
| /root/open-design/.git/objects/pack/ | 1.5 GB | Git repo pack | 🟡 Archive or shallow clone |
| /root/.cache/huggingface/ | 1.2 GB | Embedding models | Keep (needed) |
| /var/log/syslog + syslog.1 | 1.1 GB | System logs | ✅ **ROTATE/TRUNCATE** |
| /root/.hermes/skills/.curator_backups | 0.93 GB | Curator backups | ✅ **ARCHIVE/DELETE** |
| /tmp/pip-unpack-ehtplt1z/ | 0.85 GB | Pip temp files | ✅ **DELETE** |
| /root/.cache/uv/ | 0.67 GB | UV package cache | ✅ **CLEAR** |
| /root/.hermes/checkpoints/ | 0.60 GB | Model checkpoints | 🟡 Archive old ones |
...
| /root/.hermes/node/ | ~2.3 GB | node_modules | 🟡 Verify if still needed |
...
rm -rf /root/backups/hermes-memory-cutover-20260625_1810/
...
# rm -rf /root/.hermes/skills/.curator_backups/
...
# 13. /root/open-design — if not needed:
# git -C /root/open-design gc --aggressive --prune=now
# Or: remove entirely if deprecated (~3.4 GB)

# 14. /root/.hermes/node/ — verify if needed:
# du -sh /root/.hermes/node/node_modules/
# If deprecated, rm -rf saves ~2.3 GB

# 15. /root/.hermes/checkpoints/ — archive old model checkpoints:
# tar -czf /tmp/old-checkpoints.tar.gz /root/.hermes/checkpoints/old-dir/
# rm -rf /root/.hermes/checkpoints/old-dir/
```
...
2. gbrain server daemon (`gbrain serve`) reads config from `GBRAIN_HOME=/root/.gbrain`  
3. The CLI command does NOT inherit the server's config source

**Investigation needed**:
```bash
# Check gbrain config file
cat /root/.gbrain/.env 2>/dev/null
cat /root/.gbrain/config.toml 2>/dev/null
cat /root/.gbrain/gbrain.toml 2>/dev/null
...
| 7 | deploy audit path | /root/.agent default | DOCUMENTED (use AGENT_HOME= prefix) |
...
**Root cause**: gbrain CLI reads `OPENAI_BASE_URL` and `OPENAI_API_KEY` from environment at invocation time. The gbrain serve daemon is spawned by the Hermes gateway (PID 2067147) which does NOT have these env vars set. The systemd unit `gbrain.service` has them configured but is `enabled=disabled` — the actual process is started by Hermes, not systemd.
...
- Any future `gbrain embed` should be run as: `OPENAI_BASE_URL=http://127.0.0.1:8766/v1 OPENAI_API_KEY=sk-local-dummy gbrain embed ...`
